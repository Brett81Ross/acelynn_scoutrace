...
module.exports=async function handler(req,res){
  ...
  try{
    const proto=String(req.headers['x-forwarded-proto']||'https').split(',')[0].trim(),host=req.headers.host;if(!host)throw new Error('Missing host');
    ...
    const scripts=['<script src="/demo-config.js" defer></script>','<script src="/native-install.js" defer></script>'];
    for(const script of scripts){const src=script.match(/src="([^"]+)/)?.[1];if(src&&!html.includes(src))html=html.includes('</body>')?html.replace('</body>',script+'</body>'):html+script}
